...
export const utilitiesScript = `
        ...
        function exportSVG() {
            ...
            clone.style.backgroundColor = getComputedStyle(document.body).backgroundColor;
        ...
        function exportMarkdown() {
            ...
            const workflows = typeof workflowGroups !== 'undefined' ? workflowGroups : [];

            let md = '# Workflow Analysis\\n\\n';
            ...
            md += '- **LLM Calls:** ' + nodes.filter(n => n.type === 'llm').length + '\\n';
            ...
            workflows.forEach(wf => {
                ...
                const wfNodes = nodes.filter(n => wf.nodes.includes(n.id));
                md += '### Nodes\\n\\n';
                md += '| Type | Label | Description |\\n';
                md += '|------|-------|-------------|\\n';
                wfNodes.forEach(n => {
                    const desc = (n.description || '-').replace(/\\|/g, '\\\\|').substring(0, 50);
                    md += '| ' + n.type + ' | ' + n.label + ' | ' + desc + ' |\\n';
                });
                md += '\\n';
            });
